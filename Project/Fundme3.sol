...
In Solidity, when you deploy a smart contract, changing or reading data from the blockchain's storage is incredibly expensive. constant completely changes how the variable is stored and accessed.
    ...
    */
    ...
    function fund() public payable {
        require(getConversionRate(msg.value)>= MINIMUM_USD, "You need to spend more ETH!"); // here right now the issue is 5 will be seen as 5 wei by the compiler so u gotta convert that to USD
...
// This function calls the Chainlink oracle to fetch the current price of 1 ETH in USD.
  ...
  /* 
  lets understand the above function math

  Let's pretend 1 ETH = $3,000 USD and a user is funding your contract with 2 ETH.ethAmount (2 ETH): Inside Solidity, this is stored in Wei (18 decimals).
  It looks like: 2,000,000,000,000,000,000 (2e18)
  ethPrice (Price of 1 ETH): Your getPrice() function took Chainlink's 8 decimals and padded it with 1e10 to make it 18 decimals.
  It looks like: 3,000,000,000,000,000,000,000 (3000e18)

  so now it will be (3000e18 * 2e18)1e18 = 6000e18

  so basically just remember the standard unit of solidity is wei(18 decimals) so for 6000 usd we convert it into 18 decimals so 6000e18 else compiler wont understand

  WHY WE USED VIEW IN FUNCTION HERE
  
  Yes, spot on! Saving gas is the main reason, but it also tells the Solidity compiler exactly what this function is allowed to do.

In Solidity, functions that don't modify any data on the blockchain are special. By marking getConversionRate as view, you are telling the Ethereum network: "This function is read-only. It looks at the blockchain (to check the price), but it doesn’t change anything."
